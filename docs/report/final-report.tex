...
The \href{https://www.pratham.org/about/}{Pratham Education Foundation} operates ASER (Annual Status of Education Report), the world's largest citizen-led rural learning assessment. Every year, tens of thousands of field workers collect handwritten student assessment responses across India. These responses are written in Hindi on paper in widely varying handwriting styles.
...
\subsubsection{Why Existing Tools Fail}
...
\begin{itemize}
    \item \textbf{Section 2:} Technical architecture and pipeline design
    \item \textbf{Section 3:} Dataset descriptions and curation methodology
    \item \textbf{Section 4:} YOLO-based word detection (Stage 1)
    \item \textbf{Section 5:} TrOCR-based word recognition (Stage 2)
    \item \textbf{Section 6:} Post-processing techniques (Stage 3)
    \item \textbf{Section 7:} Results and evaluation methodology
    \item \textbf{Section 8:} Attribution analysis: what each technique contributed
    \item \textbf{Section 9:} Failure modes and deployment constraints
    \item \textbf{Section 10:} Conclusions and future work
\end{itemize}
\newpage
% ============================================================================
...
\subsubsection{Stage 1: Word Detection (YOLO)}
...
\subsubsection{Stage 2: Word Recognition (TrOCR)}
...
\subsubsection{Stage 3 \& 4: Post-Processing}
...
\subsubsection{Why Decouple Detection and Recognition?}
...
\newpage
% ============================================================================
\section{Datasets and Data Curation}
% ============================================================================
...
\subsection{Pratham Field Dataset}
...
\subsubsection{Annotation Quality}
...
\subsection{Domain-Specific Adaptation Data}
...
\newpage
% ============================================================================
\section{Stage 1: YOLO-Based Word Detection}
% ============================================================================

\subsection{YOLO Architecture and Fine-Tuning}
...
\subsubsection{Training Configuration (v2)}
...
\subsubsection{Matra-Aware Loss Weighting}
...
\subsubsection{Box Merging Post-Processing}
...
\newpage
% ============================================================================
\section{Stage 2: TrOCR-Based Word Recognition}
% ============================================================================

\subsection{TrOCR Architecture}
...
\subsubsection{v1 Training (HindiSeg only)}
...
\subsubsection{v2 Training (Domain Adaptation)}
...
\begin{table}[h]
    ...
    \begin{tabular}{lrr}
        ...
        Improvement & — & $-$1.05 \% ($-$12.9\% relative) \\
        \bottomrule
    \end{tabular}
...
\newpage
% ============================================================================
\section{Stage 3: Post-Processing Techniques}
% ============================================================================
...
NFC normalization contributed $-$1.7 \% CER improvement on evaluation pages.
...
\subsubsection{LM Performance Analysis}
...
\subsubsection{Dictionary Constraint}
...
Dictionary constraint contributed $-$0.7 \% CER on evaluation pages, providing modest improvement while enforcing domain plausibility.

\subsubsection{Reading Order Correction}
...
Reading order correction and box merging together contributed $-$3.8 \% CER by handling dense matra rows correctly.
...
\subsection{v1 Baseline Results}
...
\subsubsection{Recognition Improvements}
...
\begin{table}[h]
    ...
    \begin{tabular}{lrrr}
        ...
        \midrule
        HindiSeg test & 8.17\% & 7.12\% & $-$1.05 \% ($-$12.9\%) \\
        \bottomrule
    \end{tabular}
    \caption{TrOCR recognition v1 vs v2.}
\end{table}

\subsubsection{End-to-End System Results}
...
\begin{table}[h]
    ...
    \begin{tabular}{lrrr}
        ...
        \textbf{Setting} & \textbf{v1 CER} & \textbf{v2 CER} & \textbf{Improvement} \\
        \midrule
        Eval pages 31-41 & 51.78\% & 36.4\% & $-$15.38\% ($-$29.7\% relative) \\
        Pratham field scans & 98.54\% & 71.3\% & $-$27.24\% ($-$27.6\% relative) \\
        \bottomrule
    \end{tabular}
    \caption{End-to-end system v1 vs v2.}
\end{table}

\subsubsection{Word-Level Accuracy}
...
\newpage
% ============================================================================
\section{Attribution Analysis: Per-Technique Contribution}
% ============================================================================

\subsection{Evaluation Pages (Controlled Setting)}

This section breaks down the contribution of each technique to the total $-$15.38\% CER improvement on evaluation pages 31-41.

\begin{table}[h]
    \centering
    \begin{tabular}{lrl}
        \toprule
        \textbf{Technique} & \textbf{Δ CER (\% est.)} & \textbf{Notes} \\
        \midrule
...
\subsection{Pratham Field (Real-World Messy Data)}
...
\subsection{Key Insight: Domain Adaptation Is Essential}

The 16.4\% improvement from domain adaptation (the single largest factor) demonstrates that handwritten Hindi OCR cannot rely solely on general-purpose datasets. Real-world ASER data requires specialized training.

However, only 240 annotated samples were available for adaptation --- a limitation acknowledged in the report and future work.
\newpage
% ============================================================================
\section{Failure Modes and Deployment Constraints}
% ============================================================================

\subsection{Known Failure Modes}

\subsubsection{Failure 1: Box Over-Merging (Detection)}
...
\subsubsection{Failure 2: Language Model Degradation on Field Data}
...
\textbf{Evidence:} +0.6\% regression when LM is enabled on Pratham field.
...
\subsubsection{Failure 3: Inter-Annotator Agreement (Quality)}
...
\subsubsection{Failure 4: Reading Order Failures}
...
\subsection{Word-Exact Match Accuracy: Why 11.8\% is Critical}
...
\subsection{Field Performance Analysis}
...
\subsubsection{v2 Improvements Are Genuine}

Despite challenges, v2's 27.2\% improvement (98.54\% $\to$ 71.3\% CER) is substantial:
\begin{itemize}
...
\subsection{Deployment Constraints (Non-Negotiable)}

\subsubsection{Constraint 1: Human-in-the-Loop Is Mandatory}
...
\subsubsection{Constraint 2: Gold Set Quality Matters}

Expanding domain adaptation to 400+ carefully annotated examples could improve field CER by an estimated 5-10\%. Currently limited to 140 high-confidence rows.

\subsubsection{Constraint 3: Post-Processing Is Domain-Specific}
...
% ============================================================================
\section{System Integration and Pipeline}
% ============================================================================

\subsection{End-to-End Pipeline (v2)}
...
The pipeline is configured via \texttt{.env} file:
...
\subsection{HITL (Human-in-the-Loop) Review System}
...
\begin{itemize}
    ...
    \item \textbf{Domain adaptation:} Domain mix-in contributed 16.4 \% improvement on field data
    \item \textbf{Post-processing:} Integrated NFC, LM, dictionary, and reading order with careful ablation
\end{itemize}

\subsubsection{Infrastructure and Tools}
...
\subsubsection{What Didn't Achieve Target}
...
\subsubsection{Lesson 1: Domain Adaptation Is Essential}

The single largest improvement (16.4 \%) came from training on 240 field-annotated samples. General-purpose models, even fine-tuned ones, cannot match domain-specific training.

\subsubsection{Lesson 2: Post-Processing Must Match Domain}

The LM, trained on HindiSeg vocabulary, regressed on field data (+0.6 \% CER). Domain-specific post-processing (dictionary, LM) requires domain-specific tuning. A general-purpose LM may hurt.

\subsubsection{Lesson 3: Data Quality Limits Learning}
...
\subsubsection{Lesson 4: Decoupled Architecture Enables Transparency}
...
\subsubsection{Short-term (Production Hardening)}
...
\subsubsection{Medium-term (Model Improvements)}
...
\subsubsection{Long-term (Fundamental Research)}
...
\subsection{Pratham Field Format (CSV)}
...
\subsection{Recognition Error Distribution}
